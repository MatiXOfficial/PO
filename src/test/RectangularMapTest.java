...
public class RectangularMapTest
{
    @Test public void test1()
    {
        ...
        Assert.assertEquals(new Vector2d(1, 3), animals.get(2).getPosition());
        Assert.assertEquals(MapDirection.WEST, animals.get(2).getOrientation());

        Assert.assertEquals(new Vector2d(8, 8), animals.get(3).getPosition());
        Assert.assertEquals(MapDirection.NORTH, animals.get(3).getOrientation());

        Assert.assertEquals(new Vector2d(1, 1), animals.get(4).getPosition());
